...
function begin_play()
{
	...
	if (isEmpty(p1.value) == true || isEmpty(p2.value) == true)
	{
	...
	{
		p1.placeholder = p1.value += " (X)";
		p2.placeholder = p2.value += " (O)";
		p1.disabled = true;
		p2.disabled = true;
		started = true;
		game_started();
		document.getElementById("turn_info").innerHTML = p1.value + "'s turn";
	}
}
...
function reset_play()
{
	...
	document.getElementById("player1_id").placeholder = "player1_name";
	document.getElementById("player2_id").placeholder = "player2_name";
...
function play()
{
	...
	var flag = false;
	var winner;
	for (loop = 0; loop < table_ids.length; loop++) // check for valid move
	{
		if (m.value == table_ids[loop] && board_state[loop] == -1)
		{
			valid = true;
			board_state[loop] = turn;
		}
	}
	...
	{
		document.getElementById(m.value).innerHTML = "X";
		document.getElementById("turn_info").innerHTML = p2.value + "'s turn";
		turn++;
		toggle_move();
	}
	else
	{
		document.getElementById(m.value).innerHTML = "O";
		document.getElementById("turn_info").innerHTML = p1.value + "'s turn";
		turn++;
		toggle_move();
	}
	...
	if (board_state[2] == board_state[4] && board_state[0] == board_state[6] && board_state[2] != -1)
	{
		winner = board_state[2];
		if (winner == 1)
		{
